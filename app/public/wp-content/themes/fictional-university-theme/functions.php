...
{   //第一引数はカスタムしたいpost-type、第２引数は追加したいproperty-name
    ...
    register_rest_field('note', 'userNoteCount', array(
        'get_callback' => function () {
            return count_user_posts(get_current_user_id(), 'note'); //userIdに紐付いているpostの数を返す
        }
    ));
}
//REST APIをカスタマイズする
...
function university_files()
{
    ...
    if (strstr($_SERVER['SERVER_NAME'], 'fictional-university.local')) {
        ...
        wp_enqueue_script('main-university-js', get_theme_file_uri('/bundled-assets/scripts.4c1eee013172c6bbf33f.js'), NULL, '1.0', true);
        wp_enqueue_style('our-main-styles', get_theme_file_uri('/bundled-assets/styles.4c1eee013172c6bbf33f.css'));
    }
...
function makeNotePrivate($data, $postarr) //第２引数でpostのidを受け取ることができる
{
    if ($data['post_type'] == 'note') {
        if (count_user_posts(get_current_user_id(), 'note') > 4 and !$postarr['ID']) { //新規作成時にのみ有効
